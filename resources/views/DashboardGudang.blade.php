...
<body class="bg-slate-200 min-h-screen flex items-center justify-center p-0 sm:p-4">
    ...
    <div class="w-full max-w-[420px] bg-[#f8fafb] min-h-screen sm:min-h-[840px] shadow-2xl relative flex flex-col justify-between overflow-hidden sm:rounded-3xl border border-slate-200">
        ...
        <div class="overflow-y-auto pb-24">
            ...
            @php
                // 1. Ambil data produk jika tidak dikirim dari controller
                $semuaProduk = $products ?? (class_exists('App\Models\Product') ? \App\Models\Product::all() : collect());
                
                // 2. Hitung jumlah produk yang stoknya <= 10 secara akurat
                $hitungKritis = $semuaProduk->where('stock', '<=', 10)->count();
                
                // 3. Gunakan hitungan dinamis jika $stokKritisCount dari controller bernilai 0 atau null
                $stokKritisAkurat = (!empty($stokKritisCount) && $stokKritisCount > 0) ? $stokKritisCount : $hitungKritis;

                // 4. Hitung total SKU
                $totalSkuAkurat = $totalSku ?? $semuaProduk->count();
            @endphp
            ...
            <main class="p-4 space-y-4">
                ...
                <div class="bg-[#e0e8f9] border-l-4 border-red-500 rounded-r-2xl p-3.5 flex items-start gap-3 shadow-sm">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-600 font-medium leading-relaxed mt-0.5">
                            <span class="font-bold text-slate-700">{{ $stokKritisAkurat }} SKU</span> membutuhkan perhatian / restock.
                        </p>
                    </div>
                </div>
                ...
                <div>
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <span class="text-xs font-semibold text-slate-500">Total: <strong class="text-slate-700">{{ $totalSkuAkurat }} SKU</strong></span>
                    </div>
                        ...
                        @forelse($barangHariIni ?? [] as $item)
                            <div class="bg-white border border-slate-200/90 rounded-2xl p-4 shadow-sm hover:border-slate-300 transition">
                                <div class="text-[12px] font-bold text-[#029668] tracking-wide mb-1 font-mono-custom">
                                    {{ isset($item->created_at) ? \Carbon\Carbon::parse($item->created_at)->format('H:i') : date('H:i') }}
                                </div>
                                <h4 class="text-sm font-extrabold text-slate-800 tracking-tight mb-3">
                                    {{ $item->name ?? $item->product_name ?? 'Nama Produk' }}
                                </h4>
                                ...
                                <div class="border-t border-slate-100 pt-2.5 flex items-center justify-between text-xs">
                                    <div class="flex items-center gap-1.5 text-slate-500 font-medium">
                                        ...
                                        <span>QTY: <strong class="text-slate-700">{{ $item->stock ?? 0 }} Pcs</strong></span>
                                    </div>
                                    <a href="{{ Route::has('kelola.gudang') ? route('kelola.gudang') : url('/kelola') }}" class="text-[#024d35] font-bold hover:underline flex items-center gap-1">
                                        <span>Details</span>
        ...
        <nav class="absolute bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-3 py-2 flex items-center justify-around z-30">
            ...
            <a href="{{ Route::has('dashboard.gudang') ? route('dashboard.gudang') : '#' }}" class="flex flex-col items-center justify-center bg-[#00f0aa] text-[#024d35] px-4 py-1.5 rounded-xl font-bold text-[10px]">
                <i class="fa-solid fa-border-all text-base mb-0.5"></i>
                <span>Dashboard</span>
            </a>

            <!-- Tab Kelola -->
            <a href="{{ Route::has('kelola.gudang') ? route('kelola.gudang') : '#' }}" class="flex flex-col items-center justify-center text-slate-500 hover:text-[#024d35] py-1 text-[10px] font-bold transition">
            ...
            <!-- Tab Kritis -->
            <a href="{{ Route::has('stok.kritis') ? route('stok.kritis') : '#' }}" class="flex flex-col items-center justify-center text-slate-500 hover:text-[#024d35] py-1 text-[10px] font-bold transition relative">
                <div class="relative">
                    <i class="fa-solid fa-triangle-exclamation text-base mb-0.5"></i>
                    @if($stokKritisAkurat > 0)
