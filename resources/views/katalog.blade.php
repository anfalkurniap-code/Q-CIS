...
<body class="bg-gray-100 flex justify-center items-center min-h-screen">
    ...
    <div class="w-full max-w-md bg-white min-h-screen flex flex-col relative shadow-2xl overflow-hidden rounded-3xl border border-gray-200" x-data="martApp()">
        ...
        <header class="px-5 pt-6 pb-4 bg-white sticky top-0 z-10 border-b border-gray-100">
            ...
            <div class="relative mb-4">
                ...
                <input type="text" x-model="searchQuery" class="w-full pl-10 pr-4 py-2.5 bg-sky-50/50 border border-transparent focus:border-emerald-500 focus:bg-white rounded-xl text-sm outline-none transition" placeholder="Cari produk di mart...">
            </div>

            <div class="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                <template x-for="cat in categories" :key="cat">
                    <button @click="selectedCategory = cat" :class="selectedCategory === cat ? 'bg-emerald-800 text-white' : 'bg-sky-50 text-sky-800 hover:bg-sky-100'" class="px-4 py-1.5 rounded-full text-xs font-semibold transition shrink-0" x-text="cat"></button>
                </template>
            </div>
        </header>
            ...
            <div class="grid grid-cols-2 gap-4">
                <template x-for="product in filteredProducts()" :key="product.id">
                    <div class="bg-white border border-gray-100 rounded-2xl p-4 flex flex-col justify-between shadow-sm relative hover:border-emerald-200 transition">
                        <span :class="product.stock > 5 ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-600'" class="absolute top-3 right-3 text-[10px] font-bold px-2 py-0.5 rounded-md" x-text="product.stock > 5 ? 'TERSEDIA' : 'HAMPIR HABIS'"></span>
                        <div class="w-full h-24 bg-slate-50 rounded-xl mb-3 flex items-center justify-center text-xs text-slate-400 font-medium border border-dashed border-slate-200">Tanpa Gambar</div>
                        <div class="mb-3">
        ...
        <div class="absolute bottom-16 left-0 right-0 bg-white border-t border-gray-100 px-5 py-3 shadow-[0_-4px_10px_rgba(0,0,0,0.03)] z-20">
            <div class="flex justify-between items-center">
                ...
                <button @click="checkout()" :disabled="cart.length === 0" :class="cart.length === 0 ? 'bg-gray-300 cursor-not-allowed' : 'bg-emerald-800 hover:bg-emerald-700'" class="px-5 py-2.5 text-white rounded-xl font-bold text-sm flex items-center gap-2 transition">
                    <span>Checkout</span>
    ...
    <script>
        function martApp() {
            return {
                ...
                products: [
                    { id: 1, name: 'Susu Ultramilk Strawberry 250ml', price: 6000, stock: 24, category: 'Minuman' },
                    { id: 2, name: 'Roti Tawar Sari Roti', price: 6500, stock: 15, category: 'Makanan Ringan' }
                ],
                cart: [],
