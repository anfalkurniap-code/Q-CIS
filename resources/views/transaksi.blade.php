        ...
        <div>
            ...
            <div class="bg-gray-50/60 p-4 rounded-2xl border border-gray-100 text-xs">
                ...
                <div class="space-y-2.5 mb-4">
                    @foreach($transaction->items as $item)
                        <div class="flex justify-between items-center text-gray-600">
                            <span>{{ $item->nama }}</span>
                            <span class="font-semibold text-gray-800">Rp {{ number_format($item->harga, 0, ',', '.') }}</span>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="border-t border-gray-200 pt-3 mt-3 flex justify-between font-bold text-sm text-gray-800">
                    ...
                    <span class="text-emerald-700">Rp {{ number_format($transaction->total, 0, ',', '.') }}</span>
                </div>

                <div class="border-t border-gray-200 pt-3 mt-3 space-y-1 text-[11px] text-gray-400">
                    <div class="flex justify-between">
                        <span>ID Transaksi</span>
                        <span class="font-mono text-gray-600">{{ $transaction->id }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span>Waktu Transaksi</span>
                        <span class="text-gray-600">{{ $transaction->created_at }}</span>
                    </div>
                </div>
            </div>
        </div>

        <!-- TOMBOL SELESAI -> LANGSUNG KEMBALI KE ROUTE KATALOG -->
        <div class="mt-6 mb-2">
            <a href="{{ route('katalog') }}" 
               class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3.5 px-4 rounded-xl block text-center shadow-lg shadow-emerald-600/20 transition">
                Selesai
            </a>
        </div>
